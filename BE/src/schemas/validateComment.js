...
const commentSchema = Joi.object({
  ...
  ProductID: Joi.string().required().messages({
    'string.empty': 'Product ID không được bỏ trống',
    'any.required': 'Product ID là trường bắt buộc',
  }),
  UserID : Joi.string().required().messages({
    'string.empty': 'User ID không được bỏ trống',
    'any.required': 'User ID là trường bắt buộc',
  }),

  CommentContent: Joi.string().required().min(10) 
